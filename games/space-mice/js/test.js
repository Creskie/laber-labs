...
var asteroidBlaster = 0;
var ncoins = 0;
var shootingStarTurn = true;
...
var Game = {
	preload: function() {
			 ...
			 game.load.image('coin', 'assets/Star_Coin.png');
	    game.load.spritesheet('heart', 'assets/hearts.png', 85, 45);
	...
	create: function () {
			...
			// Add the coins as a group
			coins = game.add.physicsGroup();
			coins.createMultiple(30, 'coin', false);
			coins.setAll('checkWorldBounds', true);
			coins.setAll('outOfBoundsKill', true);
			
			// Add the coins as a group
			hearts = game.add.physicsGroup();
			hearts.createMultiple(30, 'heart', false);
			hearts.setAll('checkWorldBounds', true);
			hearts.setAll('outOfBoundsKill', true);
	...
	//		score = 0;
			...
			counter = 1;
			prevCounter = -100; // Previous time for using shooting star 
	...
	update: function() {
			if (highScore == -99999999) {
				...
				scoreText.text = "Score: " + score + "   Lives: " + lives + "   Level: " + curlevel + "   Coins: " + ncoins + "  Stars:  " + asteroidBlaster;
			} else {
				scoreText.text = "Score: " + score + 	"  High Score: " + Game.getCookie("Hscore") + "   Lives: " + lives + "   Level: " + curlevel +
				         "   Coins: " + ncoins +  "  Stars:  " + asteroidBlaster;
			}
			...
			if (game.input.keyboard.isDown(Phaser.Keyboard.SPACEBAR)) {
				if (asteroidBlaster >= 1 && counter > prevCounter + 10) {
					prevCounter = counter; 
					asteroidBlaster -= 1;
					Game.blastAsteroid();
				}
			...
			// add overlap between player and coin
			game.physics.arcade.overlap(player, coins, this.getCoin, null, this);
	...
	bigHitMouse: function(big, mouse) {
			     mouse.kill();
			     shoot.play();
			     score += Math.round((1200 - player.x)/120);
		     },

	bigHitRat: function(big, rat) {
			   rat.kill();
			   shoot.play();
			   score += Math.round(4 + (1200 - player.x)/120);
		   },
	...
	genCoin: function() {
            //	console.log(micespeed);
			  if (counter % 800 == 0) {
				  ...
				  if (coin) {
					  coin.reset(Math.random()*game.width, Math.min(Math.random()*game.height, game.height - 100), 'coin');
				  }
			  }
		  },
	...
	genHeart: function() {
            //	console.log(micespeed);
	    if(shootingStarTurn == false) {
			  if (counter % 1013 == 0) {
				  heart = hearts.getFirstExists(false);

				  if (heart) {
				      heart.reset(-50, Math.min(Math.random()*game.height, game.height - 100), 'heart');
				      heart.body.velocity.x = 1000;
				      heart.body.velocity.y = 50;
				      shootingStar.immovable = true
				      heart.animations.add('fly');
				      heart.animations.play('fly', 2, true);
				      counter += 1;
				      shootingStarTurn = true;
				  }
			  }
	    }
		  },
	...
	if(curlevel > 2) {
			     if (counter % 617 == 0) {
					 ...
					 if (dir <= 0.125) {
						 ...
						 if (asteroid45) {
						     asteroid45.reset(Math.random()*(game.width/2), -100, 'asteroid45');
						 ...
						 if (asteroid90) {
						     asteroid90.reset(Math.random()*(game.width - 100) + 50, -100, 'asteroid90');
						 ...
						 if (asteroid135) {
							 asteroid135.reset(Math.random()*game.width + 300, -100, 'asteroid135');
						 ...
						 if (asteroid180) {
						     asteroid180.reset(1200, Math.random()*(game.height - 50) + 25, 'asteroid180');
						 ...
						 if (asteroid225) {
							 asteroid225.reset(Math.random()*game.width + 300, 600, 'asteroid225');
						 ...
						 if (asteroid270) {
						     asteroid270.reset(Math.random()*(game.width - 100) + 50, 600, 'asteroid270');
						 ...
						 if (asteroid315) {
						     asteroid315.reset(Math.random()*(game.width/2), 600, 'asteroid315');
